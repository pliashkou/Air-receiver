/*
 * JNI bridge for the AirPlay FairPlay key-decryption routine.
 *
 * The playfair_decrypt() function is a port of Apple's FairPlay crypto
 * (originally reverse-engineered in the RPiPlay project, LGPL v2.1).
 *
 * The only entry point exported to Java/Kotlin is:
 *   com.airreceiver.tv.crypto.PlayFairNative.decrypt(byte[164], byte[72]) -> byte[16]
 */

#include <jni.h>
#include <string.h>
#include <stdlib.h>
#include "playfair/playfair.h"

/* Also need the fairplay_setup / fairplay_handshake reply tables */

/* Pre-computed FairPlay Phase-1 responses (one per mode 0-3).
 * Source: RPiPlay lib/fairplay_playfair.c — reply_message[4][142] */
static const unsigned char reply_message[4][142] = {
    {},
    {},
    {},
    {}
};

/* Header used in Phase-2 (handshake) response.
 * Source: RPiPlay lib/fairplay_playfair.c — fp_header[] */
static const unsigned char fp_header[12] = {
    0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14
};

/* fairplay_setup: 16-byte request → 142-byte response */
JNIEXPORT jbyteArray JNICALL
Java_com_airreceiver_tv_crypto_PlayFairNative_setup(JNIEnv *env, jclass clazz,
                                                    jbyteArray req_arr)
{
    jsize req_len = (*env)->GetArrayLength(env, req_arr);
    if (req_len < 16) return NULL;

    jbyte *req = (*env)->GetByteArrayElements(env, req_arr, NULL);
    int mode = (unsigned char) req[14];
    (*env)->ReleaseByteArrayElements(env, req_arr, req, JNI_ABORT);

    if (mode < 0 || mode > 3) return NULL;

    jbyteArray res = (*env)->NewByteArray(env, 142);
    (*env)->SetByteArrayRegion(env, res, 0, 142, (const jbyte *) reply_message[mode]);
    return res;
}

/* fairplay_handshake: 164-byte request → 32-byte response */
JNIEXPORT jbyteArray JNICALL
Java_com_airreceiver_tv_crypto_PlayFairNative_handshake(JNIEnv *env, jclass clazz,
                                                         jbyteArray req_arr)
{
    jsize req_len = (*env)->GetArrayLength(env, req_arr);
    if (req_len < 164) return NULL;

    jbyte *req = (*env)->GetByteArrayElements(env, req_arr, NULL);
    unsigned char res[32];
    memcpy(res, fp_header, 12);
    memcpy(res + 12, req + 144, 20);
    (*env)->ReleaseByteArrayElements(env, req_arr, req, JNI_ABORT);

    jbyteArray result = (*env)->NewByteArray(env, 32);
    (*env)->SetByteArrayRegion(env, result, 0, 32, (const jbyte *) res);
    return result;
}

/*
 * fairplay_decrypt: (message3[164], cipherText[72]) → aesKey[16]
 *
 * message3 = the 164-byte Phase-2 request that was received during handshake.
 * cipherText = the 72-byte "ekey" value from the plist SETUP request.
 */
JNIEXPORT jbyteArray JNICALL
Java_com_airreceiver_tv_crypto_PlayFairNative_decrypt(JNIEnv *env, jclass clazz,
                                                       jbyteArray msg3_arr,
                                                       jbyteArray cipher_arr)
{
    jsize msg3_len   = (*env)->GetArrayLength(env, msg3_arr);
    jsize cipher_len = (*env)->GetArrayLength(env, cipher_arr);
    if (msg3_len < 164 || cipher_len < 72) return NULL;

    jbyte *msg3   = (*env)->GetByteArrayElements(env, msg3_arr, NULL);
    jbyte *cipher = (*env)->GetByteArrayElements(env, cipher_arr, NULL);

    unsigned char keyOut[16];
    playfair_decrypt((unsigned char *) msg3, (unsigned char *) cipher, keyOut);

    (*env)->ReleaseByteArrayElements(env, msg3_arr, msg3, JNI_ABORT);
    (*env)->ReleaseByteArrayElements(env, cipher_arr, cipher, JNI_ABORT);

    jbyteArray result = (*env)->NewByteArray(env, 16);
    (*env)->SetByteArrayRegion(env, result, 0, 16, (const jbyte *) keyOut);
    return result;
}
